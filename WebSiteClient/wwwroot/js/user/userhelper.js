    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success) {
                ...
                saveToken(md);
                updateViewAfLogin(data.data.user, true);
                return true;
            }
            else {
                bootbox.alert({
...
function updateViewAfLogin(user, bol) {
    if (bol) {
        $('#modalLogin').modal('toggle');
    }
    $('#h-f-sh-lg').hide();
    ...
    $('#lg-inp-email').val("");
    $('#lg-inp-pass').val("");
    $('#h-t-name-user').text(user.fullname);
    if (user.avatar != null || user.avatar != '') {
        $('.img-avt-us').css("background-image", "url(" + serverfileuser + user.avatar + ")");
    }
}
...
function checkTokenServer() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success) {
                updateViewAfLogin(data.data, false);
                return true;
            }
            else {
                return false;
